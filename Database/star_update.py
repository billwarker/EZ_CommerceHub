...
inventory_file = "InventoryData.xlsx" 
...
for row in range(2, sheet.max_row + 1):
	...
	try:
		cur.execute("INSERT INTO star_inventory VALUES (?, ?, ?, ?, ?)", item_info)
		#conn.commit()
		...
		cur.execute("SELECT item_inv FROM star_inventory WHERE item_sku = ?", (item_sku,))
		prev_inv = int(cur.fetchone()[0])
		cur.execute("UPDATE star_inventory SET item_inv = ?  WHERE item_sku = ?", (item_inv, item_sku))
